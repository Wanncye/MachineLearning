...
class RandomForestRegression(object):
    ...
    def _parallel_build_trees(self, dataset, targets, random_state):
        ...
        # dataset_stage = dataset_stage.iloc[:, subcol_index]
        dataset_stage = dataset_stage[subcol_index]
...
if __name__ == '__main__':
    df = pd.read_csv("resource/housing.txt").fillna(-1)
    ...
    dataset = df.iloc[:train_count, :len(feature_list)]
    target = df.iloc[:train_count, -1]
    clf.fit(dataset, target)

    from sklearn import metrics
    print(metrics.mean_squared_error(df.iloc[:train_count, -1], clf.predict(df.iloc[:train_count, :len(feature_list)])))
    print(metrics.mean_squared_error(df.iloc[train_count:, -1], clf.predict(df.iloc[train_count:, :len(feature_list)])))